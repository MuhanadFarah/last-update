...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['scannedData'])) {
    ...
    if (count($data) === 3) {
        ...
        $checkStudent = $conn->prepare("SELECT * FROM students WHERE student_id = ? AND name = ? AND faculty = ?");
        $checkStudent->bind_param("sss", $student_id, $student_name, $faculty);
        ...
        if ($studentResult->num_rows > 0) {
            ...
            if ($result->num_rows == 0) {
                $insert = $conn->prepare("INSERT INTO attendance (student_id, student_name, faculty, date, teacher_id) VALUES (?, ?, ?, ?, ?)");
                $insert->bind_param("ssssi", $student_id, $student_name, $faculty, $date, $teacher_id);
                $insert->execute();
        ...
        $message = "❌ Invalid QR format. Expected: student_id,student_name,faculty";
    }
}
?>
