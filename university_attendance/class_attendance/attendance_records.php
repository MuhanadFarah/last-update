...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'edit') {
        ...
        if ($student_id === '' || $student_name === '' || $faculty === '') {
            $response['message'] = "All fields are required.";
            echo json_encode($response);
            exit;
        }

        $updStmt = $conn->prepare("UPDATE attendance SET student_id = ?, student_name = ?, faculty = ? WHERE id = ?");
        $updStmt->bind_param("sssi", $student_id, $student_name, $faculty, $id);
        if ($updStmt->execute()) {
            $response = ['success' => true];
        } else {
    ...
    if ($_POST['action'] === 'add') {
        ...
        $insertStmt = $conn->prepare("INSERT INTO attendance (student_id, student_name, faculty, date, teacher_id) VALUES (?, ?, ?, ?, ?)");
        $insertStmt->bind_param("ssssi", $student_id, $student_name, $faculty, $date, $teacher_id);
      ...
      <table class="table table-bordered table-hover align-middle" id="attendanceTable">
        ...
        <tbody>
          ...
          <tr data-id="<?= $row['id'] ?>">
            ...
            <td><?= htmlspecialchars(date("H:i:s", strtotime($row['created_at'] ?? $row['date']))) ?></td>
            <td>
